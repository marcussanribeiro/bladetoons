...
"""def ranking_animes(periodo):
    ...
    return (
        Anime.objects.annotate(
            total_views=Count(
                'volumes__capitulos__viewcapitulo',
                filter=Q(volumes__capitulos__viewcapitulo__created_at__gte=inicio)
            )
        )
        .order_by('-total_views')  # 🔥 ORDEM CORRETA
        [:10]
    )"""
